...
const { init, disableOsmosis, enableOsmosis, isCloudLoggingEnabled, sendToOsmosis, __testing__ } = OsmosisCloud;
...
describe('Osmosis Cloud Module', () => {
  ...
  describe('Initialization and Configuration', () => {
    test('init() should initialize cloud logging with API key', async () => {
      ...
      expect(console.log).toHaveBeenCalledWith(expect.stringContaining('Cloud logging initialized'));
    });
    ...
    test('init() should warn when no API key provided', async () => {
      ...
      expect(console.warn).toHaveBeenCalledWith(expect.stringContaining('No API key provided'));
    });
  ...
  describe('Data Sending', () => {
    ...
    test('sendToOsmosis() should handle API errors', async () => {
      ...
      expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining('API returned status 400'));
    ...
    test('sendToOsmosis() should handle exceptions', async () => {
      ...
      expect(console.warn).toHaveBeenCalledWith(expect.stringContaining('Failed to prepare data for Osmosis API'));
    });
