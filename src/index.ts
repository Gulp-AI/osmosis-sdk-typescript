...
async function loadLibraries() {
  ...
  if (osmosisConfig.enabledAPIs?.openai) {
    try {
      ...
      console.warn('[OSMOSIS-AI] OpenAI library not available:', (error as Error).message);
    }
  }
  ...
  if (osmosisConfig.enabledAPIs?.anthropic) {
    try {
      ...
      console.warn('[OSMOSIS-AI] Anthropic library not available:', (error as Error).message);
    }
  }
  ...
  if (osmosisConfig.enabledAPIs?.langchainOpenai) {
    try {
      ...
      console.warn('[OSMOSIS-AI] LangChain OpenAI library not available:', (error as Error).message);
    }
  }
  ...
  if (osmosisConfig.enabledAPIs?.langchainAnthropic) {
    try {
      ...
      console.log('[OSMOSIS-AI] Successfully loaded LangChain Anthropic library');
      
      // Apply the patch right away if library is loaded
      patchLangchainAnthropic();
    } catch (error) {
      console.warn('[OSMOSIS-AI] LangChain Anthropic library not available:', (error as Error).message);
    }
  }
}
...
function logRequest(
  ...
  if (osmosisConfig.logDestination === 'console' || osmosisConfig.logDestination === 'both') {
  ...
  if ((osmosisConfig.logDestination === 'cloud' || osmosisConfig.logDestination === 'both') && 
...
function patchOpenAI(): void {
  ...
  OpenAI.prototype.request = function<Req, Rsp>(
    ...
    resolvedOptionsPromise.then(resolvedOptions => {
      if (osmosisConfig.logDestination === 'console' || osmosisConfig.logDestination === 'both') {
      ...
      console.log(`  [Error processing OpenAI request options]: ${err.message}`);
    });
    
    // Call the original method and get the response promise
    const responsePromise = originalOpenAIRequest!.apply(this, [options, remainingRetries]) as APIPromise<Rsp>;
    ...
    try {
      ...
      OpenAI = function(...args: any[]) {
        ...
        if (instance.chat && instance.chat.completions) {
          ...
          instance.chat.completions.create = async function(...createArgs: any[]) {
            ...
            try {
              ...
              if (!params.stream && (osmosisConfig.logDestination === 'cloud' || osmosisConfig.logDestination === 'both')) {
                OsmosisCloud.sendToOsmosis(
                  {
                    api: 'OpenAI',
                    version: 'v2',
          ...
          if (originalCreate.stream) {
            ...
            instance.chat.completions.create.stream = async function(...streamArgs: any[]) {
              ...
              if (osmosisConfig.logDestination === 'cloud' || osmosisConfig.logDestination === 'both') {
                ...
                console.log('[OSMOSIS-AI] Streaming request initiated with ID:', initialLogId);
              }
              
              // Call original stream method
              // @ts-ignore - TypeScript doesn't understand the args
              const originalStream = await originalCreate.stream.apply(this, streamArgs);
                ...
                [Symbol.asyncIterator]: async function* () {
                  try {
                    ...
                    if (streamContent.length > 0) {
                      ...
                      console.log(`[OSMOSIS-AI] Streaming completed, logging content (${streamContent.length} chars) with ID: ${initialLogId}`);
...
function patchAnthropic(): void {
  if (!osmosisConfig.enabledAPIs?.anthropic || !availableLibraries.anthropic) return;
  ...
  Anthropic.prototype.request = function<Req, Rsp>(
    ...
    resolvedOptionsPromise.then(resolvedOptions => {
      ...
      console.log(`  [Error processing Anthropic request options]: ${err.message}`);
    });
    
    // Call the original method and get the response promise
    const responsePromise = originalAnthropicRequest!.apply(this, [options, remainingRetries]);
...
function patchLangchainOpenAI(): void {
  if (!osmosisConfig.enabledAPIs?.langchainOpenai || !availableLibraries.langchainOpenai) return;
  ...
  ChatOpenAI.prototype._generate = async function(
    ...
    if (osmosisConfig.logDestination === 'console' || osmosisConfig.logDestination === 'both') {
      try {
        ...
        console.log(`  [Error processing LangChain OpenAI request]: ${(err as Error).message}`);
      }
    }
    ...
    if (osmosisConfig.logDestination === 'cloud' || osmosisConfig.logDestination === 'both') {
      try {
        ...
        const response = await originalLangchainOpenAIGenerate!.apply(this, [messages, options, runManager]);
      ...
      return originalLangchainOpenAIGenerate!.apply(this, [messages, options, runManager]);
    }
  };
}
...
function patchLangchainAnthropic(): void {
  if (!osmosisConfig.enabledAPIs?.langchainAnthropic || !availableLibraries.langchainAnthropic) return;
  ...
  ChatAnthropic.prototype._generate = async function(
    ...
    if (osmosisConfig.logDestination === 'console' || osmosisConfig.logDestination === 'both') {
      try {
        ...
        console.log(`  [Error processing LangChain Anthropic request]: ${(err as Error).message}`);
      }
    }
    ...
    if (osmosisConfig.logDestination === 'cloud' || osmosisConfig.logDestination === 'both') {
      try {
        // Call the original method and get the response
        const response = await originalLangchainAnthropicGenerate!.apply(this, [messages, options, runManager]);
      ...
      // Just call the original method without cloud logging
      return originalLangchainAnthropicGenerate!.apply(this, [messages, options, runManager]);
    }
  };
}
...
export function configure(options: OsmosisAIOptions): void {
  ...
  if (options.logDestination) {
    if (options.logDestination === 'cloud' || options.logDestination === 'both') {
  ...
  console.log(`[OSMOSIS-AI] Configuration updated: ${JSON.stringify({
    ...osmosisConfig,
...
export async function init(): Promise<{ 
  ...
  if (availableLibraries.langchainAnthropic) exports.ChatAnthropic = ChatAnthropic;
